...
object AppLastModifiedTimestampResolver {
    ...
    fun resolve(packageName: String): String {
        val line = exec("${Params.ADB_TOOL_PATH} -s ${Repository.Device.get()} shell dumpsys package $packageName | grep -i '/base.apk'").let { result ->
        ...
        return exec("${Params.ADB_TOOL_PATH} -s ${Repository.Device.get()} shell date -r $path +%s").let { result ->
